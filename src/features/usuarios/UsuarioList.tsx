...
export function UsuarioList() {
  ...
  if (loading) return <div className="p-8 text-center font-bold animate-pulse">Carregando usuários...</div>;
                ...
                <TableRow key={u.uid} className="hover:bg-muted/10 transition-colors">
                  <TableCell className="font-bold">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-muted-foreground font-medium tracking-wider uppercase">@{u.nickname}</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-sm">
                    <div className="flex flex-col">
                      <span>{u.email}</span>
                      {u.emailTipo === 'FAKE' && <span className="text-[9px] text-amber-600 font-bold uppercase">Gerado pelo sistema</span>}
                    </div>
                  </TableCell>
                  <TableCell>
                    <Badge variant={isGodProfile ? "destructive" : "outline"} className="font-black text-[10px]">
                      {role}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <Badge variant={u.status === 'ATIVO' ? 'default' : 'secondary'} className={u.status === 'ATIVO' ? 'bg-green-500 hover:bg-green-600' : ''}>
                      {u.status}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-xs text-muted-foreground font-medium">
                    {u.ultimoAcesso?.toDate ? format(u.ultimoAcesso.toDate(), "dd/MM/yyyy HH:mm") : 'Nunca'}
                  </TableCell>
                      ...
                      {!isGodProfile && (
                        <>
                          <Button 
                            variant="ghost" 
                            size="sm"
                            className={u.status === 'ATIVO' ? 'text-amber-600 hover:text-amber-700' : 'text-green-600 hover:text-green-700'}
                            onClick={() => updateStatus(u.uid, u.status === 'ATIVO' ? 'BLOQUEADO' : 'ATIVO')}
                          >
