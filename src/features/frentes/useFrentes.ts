...
export function useFrentes() {
  ...
  const addFrente = async (data: Omit<Frente, 'id'>) => {
    try {
      ...
      if (profile) {
        await logAction(profile.uid, profile.nickname || profile.name, 'CRIOU', 'FRENTE', data.nome, null, data);
      }
  ...
  const deleteFrente = async (id: string) => {
    try {
      ...
      if (profile && old) {
        await logAction(profile.uid, profile.nickname || profile.name, 'EXCLUIU', 'FRENTE', old.nome, old, null);
      }
