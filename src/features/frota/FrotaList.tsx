...
export function FrotaList() {
  ...
  if (loading) return <div className="p-8 text-center font-bold animate-pulse">Carregando frota...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <h3 className="text-xl font-black uppercase tracking-tight px-1 flex items-center gap-2">
          ...
          <TableBody>
            {filteredFrotas.map((frota: Frota) => (
              <TableRow key={frota.id} className="hover:bg-muted/10 transition-colors">
                <TableCell className="text-xl font-black text-primary flex items-center gap-2">
                  <Truck className="w-6 h-6 inline-block" />
                  {frota.frota}
                </TableCell>
                <TableCell className="font-black text-sm uppercase tracking-tighter text-muted-foreground">{frota.placa}</TableCell>
                <TableCell className="font-bold text-sm">
                  <div className="flex flex-col">
                    <span>{frota.nome}</span>
                    <span className="text-[10px] text-muted-foreground uppercase">{frota.marca} {frota.modelo}</span>
                  </div>
                </TableCell>
                <TableCell>
                      ...
                      <>
                        ...
                        {frota.status === 'OFICINA' ? (
                          ...
                          <Button 
                            ...
                            title={frota.status === 'ALOCADO' ? 'Frota em Operação' : 'Enviar para Oficina'}
                          >
                            <Wrench size={16} />
                          </Button>
                        )}
      ...
      <Dialog open={isWorkshopDialogOpen} onOpenChange={setIsWorkshopDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="py-4 space-y-4">
              <div className="bg-muted/50 p-4 rounded-lg space-y-2 border">
                <div className="flex justify-between">
                  <span className="text-[10px] font-black uppercase text-muted-foreground">Frota:</span>
                ...
                <div className="flex justify-between">
                  <span className="text-[10px] font-black uppercase text-muted-foreground">Placa:</span>
                  <span className="font-bold">{frotaForWorkshop.placa}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[10px] font-black uppercase text-muted-foreground">Equipamento:</span>
                  <span className="font-medium text-xs text-right">{frotaForWorkshop.nome}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black uppercase text-muted-foreground">Status atual:</span>
                  <FrotaStatusBadge status={frotaForWorkshop.status} />
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black uppercase text-muted-foreground">Novo status:</span>
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setIsWorkshopDialogOpen(false)} className="font-bold uppercase">CANCELAR</Button>
            <Button 
