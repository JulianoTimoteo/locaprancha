...
export function useRelatoriosData(filters: any) {
  ...
  useEffect(() => {
    ...
    const unsubUsuarios = subscribeToUsuarios((data) => {
      setUsuarios(data);
      checkLoaded();
    });
    const unsubFrentes = subscribeToFrentes((data) => {
      setFrentes(data);
      checkLoaded();
    });
  ...
  const filteredData = useMemo(() => {
    let data = agenda.filter((r) => {
      ...
      return (
        r.usuarioId === profile?.uid ||
        r.solicitanteId === profile?.uid ||
    ...
    if (filters.usuarioId && filters.usuarioId !== "Todos") {
      data = data.filter(
        (r) => r.solicitanteId === filters.usuarioId || r.userId === filters.usuarioId,
      );
    }
  ...
  const kpis = useMemo(() => {
    ...
    const finalizadas = filteredData.filter(
      (r) => r.status === "Finalizado" || r.status === "Concluído",
    ).length;
    const emAndamento = filteredData.filter(
      (r) => r.status === "Iniciado" || r.status === "Em Trânsito",
    ).length;
    const canceladas = filteredData.filter(
      (r) => r.status === "Cancelado" || r.status === "Recusado",
    ).length;
