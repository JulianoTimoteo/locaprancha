...
export function useTransportRequests() {
  ...
  const addRequest = async (request: Omit<TransportRequest, 'id' | 'createdAt' | 'updatedAt' | 'status'>) => {
    const newRequest = {
      ...request,
  ...
  const updateStatus = async (requestId: string, status: TransportStatus) => {
    if (isOffline) {
      const updated = requests.map(r => r.id === requestId ? { ...r, status, updatedAt: new Date() } : r);
      setRequests(updated);
