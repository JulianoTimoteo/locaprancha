...
export function normalizeAgendaRecord(id: string, data: any): Reserva {
  ...
  const solicitanteNome = normalizeString(data?.solicitanteNome || data?.solicitante || data?.userName || data?.usuario, 'Não informado');
  ...
  return {
    ...
    equipamentoNome: normalizeString(data?.equipamentoNome || data?.equipamento, 'Aguardando definição'),
    
    // Logística
    data: dataVal,
    hora: horaVal,
    horarioRetirada: horaVal,
    horarioDevolucaoPrevisto: normalizeString(data?.horarioDevolucaoPrevisto || data?.previsaoRetorno || data?.horaFim, 'Não informado'),
    
    origem: origemVal,
    destino: destinoVal,
    frenteId: normalizeString(data?.frenteId || data?.frenteTrabalho || data?.frente, 'Não informada'),
    frenteTrabalho: normalizeString(data?.frenteTrabalho || data?.frenteId || data?.frente, 'Não informada'),
